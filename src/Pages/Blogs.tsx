...
const Blogs = () => {
  ...
  return (
    <div className="tracking-wide relative pb-10">
      ...
      <div className="flex items-center w-full font-medium text-xl gap-1  justify-between px-9 bg-gradient-to-r from-orange-500 to-orange-800 bg-clip-text text-transparent">
        {toggleDeletion ? <button onClick={handleMassDeletion}>Confirm Deletion</button>:<span className="flex items-center transition-all duration-200 hover:text-2xl cursor-pointer justify-center gap-1" onClick={()=> setToggleDeletion(true)}>Mass deletion <RiChatDeleteFill className="text-orange-500" /></span>}
      ...
      <div className="flex pt-10 flex-wrap justify-center">
        {currentUser.name ? (addPost && <NewPostForm  setAddPost={setAddPost}/>): (
           addPost && (
      ...
      <div className="w-full">
        {data?.data.map((blog:BlogsType)=><BlogCard setSelectedPost={setSelectedPost} massDeletion={massDeletion} toggleDeletion={toggleDeletion} blog={blog} setShowComments={setShowComments} key={blog.id}/>)}
      </div>
      {currentUser.name ? (showComments && (
        <div className="flex pt-10 flex-wrap justify-center ">
